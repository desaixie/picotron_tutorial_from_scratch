...
from flash_attn.flash_attn_interface import flash_attn_func  # CPU/MPS NOTE: Replace with torch.nn.functional.scaled_dot_product_attention or a vanilla softmax attention on non-CUDA backends.
from flash_attn.layers.rotary import apply_rotary_emb  # CPU/MPS NOTE: Swap for a pure PyTorch rotary helper to avoid FlashAttention dependency.
from flash_attn.ops.triton.layer_norm import layer_norm_fn  # CPU/MPS NOTE: Fall back to torch.nn.LayerNorm or a Python RMSNorm when Triton kernels are unavailable.
...
def flash_attention(q, k, v, causal = True):
    ...
    return flash_attn_func(q, k, v, causal=causal)  # CPU/MPS NOTE: Swap for scaled_dot_product_attention when FlashAttention kernels are unavailable.
...
def get_cos_sin(seq_length, head_dim, base=500000.0):
    ...
    dtype = torch.bfloat16  # CPU/MPS NOTE: Use torch.float32 when bf16 is unsupported.
    device = torch.device('cuda')  # CPU/MPS NOTE: Replace with torch.device("cpu" or "mps") and guard for torch.cuda.is_available().
