...
from flash_attn.flash_attn_interface import flash_attn_func  # CPU/MPS NOTE: Replace with scaled_dot_product_attention on non-CUDA systems.
...
from flash_attn.ops.triton.layer_norm import layer_norm_fn  # CPU/MPS NOTE: Swap in torch.nn.LayerNorm or Python RMSNorm without Triton.
...
def flash_attention(q, k, v, causal = True):
    ...
    return flash_attn_func(q, k, v, causal=causal)  # CPU/MPS NOTE: Swap for scaled_dot_product_attention when FlashAttention kernels are not present.
...
def get_cos_sin(seq_length, head_dim, base=500000.0):
    ...
    device = torch.device('cuda')  # CPU/MPS NOTE: Respect the active device instead of forcing CUDA.
