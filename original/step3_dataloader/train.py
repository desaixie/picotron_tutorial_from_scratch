...
if __name__ == "__main__":    
    ...
    backend = "nccl"
    torch.cuda.set_device(local_rank)  # CPU/MPS NOTE: Guard this call and prefer torch.device("cpu"/"mps") without CUDA.
    device = torch.device("cuda", local_rank)
    dtype = torch.bfloat16  # CPU/MPS NOTE: Use torch.float32 when bf16 is unsupported.

    dist.init_process_group(rank=global_rank, world_size=world_size, backend=backend, init_method=f"env://", timeout=datetime.timedelta(minutes=2))  # CPU/MPS NOTE: Prefer backend="gloo" when NCCL cannot be used.
    ...
    while trained_token < args.max_tokens:
        ...
        print(f"[rank {pgm.process_group_manager.global_rank}] Step: {step}, Loss: {loss:.4f}, "
            ...
            f"Memory usage: {torch.cuda.memory_reserved() / 1e9:.2f}GB"  # CPU/MPS NOTE: Guard torch.cuda.* calls and skip memory stats when CUDA is unavailable.
            , is_print_rank=is_wandb_rank
        )
        
        if is_wandb_rank and args.use_wandb:
            wandb.log({"loss": loss, "tokens_per_step": tokens_per_step, "tokens_per_second": tokens_per_step / step_duration,\
                "memory_usage": torch.cuda.memory_reserved() / 1e9, "trained_tokens": tokens_per_step})  # CPU/MPS NOTE: Skip or substitute memory metrics on non-CUDA setups.
