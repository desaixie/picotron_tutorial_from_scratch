...
from flash_attn.layers.rotary import apply_rotary_emb  # CPU/MPS NOTE: Implement rotary embeddings with pure PyTorch ops when running off CUDA.
from flash_attn.ops.triton.layer_norm import layer_norm_fn  # CPU/MPS NOTE: Swap for torch.nn.LayerNorm or a Python RMSNorm fallback without Triton.
...
def flash_attention(q, k, v, causal = True):
    ...
    return flash_attn_func(q, k, v, causal=causal)  # CPU/MPS NOTE: Swap for scaled_dot_product_attention when FlashAttention is unavailable.
...
def get_cos_sin(seq_length, head_dim, base=500000.0):
    ...
    dtype = torch.bfloat16  # CPU/MPS NOTE: Use torch.float32 in environments without bf16 support.
    device = torch.device('cuda')  # CPU/MPS NOTE: Respect the configured device rather than forcing CUDA.
